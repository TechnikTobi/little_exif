...
fn check_signature(file_buffer: &Vec<u8>) -> Result<Cursor<&Vec<u8>>, std::io::Error> {
    // Check the signature
    let signature_is_valid = file_buffer[0..8]
        .iter()
...
fn encode_metadata_png(exif_vec: &Vec<u8>) -> Vec<u8> {
    ...
    let ssss = (EXIF_HEADER.len() as u32 + exif_vec.len() as u32 + 1 as u32).to_string();
...
pub(crate) fn as_u8_vec(general_encoded_metadata: &Vec<u8>, as_zTXt_chunk: bool) -> Vec<u8> {
    ...
    return construct_zTXt_chunk_data(vec![0x7a, 0x54, 0x58, 0x74], &basic_png_encode_result);
}

#[allow(non_snake_case)]
fn construct_zTXt_chunk_data(prefix: Vec<u8>, basic_png_encode_result: &Vec<u8>) -> Vec<u8> {
