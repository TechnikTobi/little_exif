...
/// stored JXL images
fn starts_with_iso_bmff_signature(file_buffer: &Vec<u8>) -> bool {
    return file_buffer[0..12]
        .iter()
        .zip(ISO_BMFF_JXL_SIGNATURE.iter())
        .filter(|&(read, constant)| read == constant)
        .count()
        == ISO_BMFF_JXL_SIGNATURE.len();
}
...
fn starts_with_jxl_signature(file_buffer: &Vec<u8>) -> bool {
    return file_buffer[0..2]
        .iter()
...
fn check_signature(file_buffer: &Vec<u8>) -> Result<(), std::io::Error> {
...
fn encode_metadata_jxl(exif_vec: &Vec<u8>) -> Vec<u8> {
