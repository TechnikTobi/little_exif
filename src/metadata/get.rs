...
impl Metadata {
    ...
    pub fn get_ifd(&self, group: ExifTagGroup, generic_ifd_nr: u32) -> Option<&ImageFileDirectory> {
        self.image_file_directories
            .iter()
            .filter(|ifd| ifd.get_generic_ifd_nr() == generic_ifd_nr && ifd.get_ifd_type() == group)
            .next()
    }
...
impl Metadata {
    pub fn get_tag(&self, tag: &ExifTag) -> GetTagIterator {
    ...
    pub fn get_tag_by_hex(&self, hex: u16, group: Option<ExifTagGroup>) -> GetTagIterator {
        GetTagIterator {
            metadata: self,
            current_ifd_index: 0,
            current_tag_index: 0,
