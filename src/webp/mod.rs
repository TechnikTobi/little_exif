        ...
        == RIFF_SIGNATURE.len()
    {
        return io_error!(
            ...
            format!(
                ...
                from_u8_vec_macro!(String, &file_buffer[0..4].to_vec(), &Endian::Big)
            )
        );
    }
...
fn check_webp_signature(file_buffer: &Vec<u8>) -> Result<(), std::io::Error> {
    if !file_buffer[8..12]
        .iter()
        ...
        == WEBP_SIGNATURE.len()
    {
        return io_error!(
            ...
            format!(
                ...
                from_u8_vec_macro!(String, &file_buffer[8..12].to_vec(), &Endian::Big)
            )
        );
    }
...
fn check_byte_count(file_buffer: &Vec<u8>, opt_file: Option<&File>) -> Result<(), std::io::Error> {
    let byte_count = from_u8_vec_macro!(u32, &file_buffer[4..8].to_vec(), &Endian::Little) + 8;
...
fn encode_metadata_webp(exif_vec: &Vec<u8>) -> Vec<u8> {
