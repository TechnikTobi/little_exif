...
fn encode_metadata_jpg(exif_vec: &Vec<u8>) -> Vec<u8> {
...
fn check_signature(file_buffer: &Vec<u8>) -> Result<(), std::io::Error> {
    // Check the signature
    let signature_is_valid = file_buffer[0..2]
        .iter()
...
pub(crate) fn file_clear_segment(path: &Path, segment_marker: u8) -> Result<(), std::io::Error> {
