...
Metadata
{
	...
	-> Result<Vec<ExifTag>, std::io::Error>
	{
		...
		let test = Self::decode_ifd(
			&mut VecDeque::from(encoded_data[14..].to_vec()),
			&ExifTagGroup::IFD0,
			8,
			&Endian::Little
		);
	...
	-> Result<Vec<ExifTag>, std::io::Error>
	{
		...
		let number_of_entries = from_u8_vec_macro!(u16, &[
			encoded_data.pop_front().unwrap(),
			encoded_data.pop_front().unwrap()
		].to_vec(), endian);
		...
		for i in 0..number_of_entries
		{
			...
			let hex_tag = from_u8_vec_macro!(u16, &[
				encoded_data.pop_front().unwrap(),
				encoded_data.pop_front().unwrap()
			].to_vec(), endian);
			let hex_format = from_u8_vec_macro!(u16, &[
				encoded_data.pop_front().unwrap(),
				encoded_data.pop_front().unwrap()
			].to_vec(), endian);
			let hex_component_number = from_u8_vec_macro!(u32, &[
				encoded_data.pop_front().unwrap(),
				encoded_data.pop_front().unwrap(),
				encoded_data.pop_front().unwrap(),
				encoded_data.pop_front().unwrap()
			...
			{
				// Compute the offset
				let hex_offset = from_u8_vec_macro!(u32, &four_byte_data, endian) - given_offset;
				raw_data = Vec::from_iter(encoded_data.range(
