...
fn read_exif_data_png1_old_version_fails() {
...
#[test]
#[should_panic(
    expected = "called `Result::unwrap()` on an `Err` value: Custom { kind: Other, error: \"No metadata found!\" }"
)]
fn read_exif_data_png1_current_version_fails() {
...
#[test]
#[should_panic(expected = "assertion failed: metadata.is_err()")]
fn read_exif_data_png2_old_version_fails() {
...
#[test]
fn read_exif_data_png2_new_version_works() {
    let png_path = Path::new("resources/issue_000052/test2.png");
    let _ = little_exif_0_6_3::metadata::Metadata::new_from_path(png_path).unwrap();
}

#[test]
fn read_exif_data_png2_current_version_works() {
    let png_path = Path::new("resources/issue_000052/test2.png");
    let _ = little_exif::metadata::Metadata::new_from_path(png_path).unwrap();
}
...
#[test]
#[should_panic(expected = "assertion failed: metadata.is_err()")]
fn read_exif_data_png3_old_version_fails() {
...
fn read_exif_data_png3_current_version_fails() {
    let png_path = Path::new("resources/issue_000052/test3.png");
    let _ = little_exif::metadata::Metadata::new_from_path(png_path).unwrap();
}
