...
fn main() -> Result<(), std::io::Error> {
    ...
    let image_description_by_tag = metadata
...
fn fill_metadata(metadata: &mut Metadata) {
    ...
    metadata.set_tag(ExifTag::ImageDescription(
        "-w 1000 -h 1000 --x_mid=0 --y_mid=0 -z 0.5 -i 1000 -c 8".to_string(),
    ));

    metadata.set_tag(ExifTag::ISO(vec![2022]));
    ...
    metadata.set_tag(ExifTag::FNumber(vec![1.4.into()]))
}
